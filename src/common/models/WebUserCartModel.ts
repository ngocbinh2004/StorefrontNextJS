...
import {
  ...
  WebUserCartItem,
  CartPrice,
} from "../types/WebUserCart";
import BaseModel from "./BaseModel";

class WebUserCartModel
  extends BaseModel
  implements BaseModelJson<WebUserCartJson>
{
  uuid: string;
  ...
  constructor(json: WebUserCartJson) {
    super();
  ...
  static getDefaultData(): WebUserCartJson {
    return {
      uuid: "",
      details: [],
      price_sell: 0,
  ...
  static calculatePrices(cart: WebUserCartJson): CartPrice {
    let priceSell = 0;
    //currently not use ^^!
    let priceHandling = 0;
    let priceFinal = 0;
  ...
  static getItemImageUrl(item: WebUserCartItem): string {
    return item.image;
  }
}

export default WebUserCartModel;
